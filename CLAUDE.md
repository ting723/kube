...
- **2026-05-06**: v0.0.6 文档完善 - 补充详细的变更日志和功能说明
- **2026-05-03**: v0.0.6 发布 - 大规模功能增强与稳定性修复

### v0.0.6 新功能

- **多窗格日志查看器**：V 追加窗格 / W 关闭当前 / Tab 切换焦点，支持多 Pod 日志并行对比
- **批量标记操作**：v 进入批量模式 / Space 标记 / d 批量删除，标记项 green 高亮，支持全部 11 种资源类型
- **日志内搜索**：/ 开启搜索 / Enter 确认 / n N 跳转匹配 / Esc 清除，关键词高亮显示
- **表格列排序**：> 键循环切换排序列
- **Context/Cluster 切换**：C 键打开集群选择器，居中弹窗叠加层交互
- **Port-forward 管理**：P 启动端口转发 / Ctrl+P 停止，使用 nohup 后台进程管理，退出时自动清理
- **Pod 选择器弹窗**：分屏日志模式下通过弹窗交互选择右侧 Pod
- **可配置快捷键**：从 `~/.config/kube-tui/keys.json` 加载自定义键位绑定
- **自动刷新/滚动状态指示器**：标题栏显示自动刷新和滚动状态
- **中英文国际化**：I 键切换界面语言
- **文本选择/鼠标滚动双模式**：M 键切换，条件鼠标捕获

### v0.0.6 修复

- 日志搜索框允许输入 'n'/'N' 字符，仅在确认后触发跳转
- 多窗格模式下活动窗格黄色边框高亮标识
- 单窗格日志自动滚动状态始终可见
- R 键在内容视图中触发即时刷新
- 内容视图（Describe/YamlView/TopView）方向键滚动
- Pod 状态显示与 `kubectl get pod` 结果一致性
- Port-forward 使用 nohup 后台运行，避免阻塞主循环
- 删除命令在主事件循环中执行，提升可靠性
- Namespace 切换时清理所有缓存状态（窗格/搜索/排序/标记）
- Pod 错误状态检测优化
