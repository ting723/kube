...
pub fn render(f: &mut Frame, area: Rect, app: &AppState) {
    ...
    let search_input = Paragraph::new(input_text)
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title("Search (type to search, ↑↓ to navigate, Enter to select, Esc to cancel)"),
        )
    ...
    if app.search_results.is_empty() {
        ...
        let items: Vec<ListItem> = match app.previous_mode {
            ...
            AppMode::DeploymentList => app
                ...
                .map(|&index| {
                    if let Some(deployment) = app.deployments.get(index) {
                        ...
                        ListItem::new(format!("{} - {}", deployment.name, deployment.ready))
                            .style(style)
                    } else {
                        ListItem::new("Invalid index").style(Style::default().fg(Color::Red))
                    }
            ...
            AppMode::DaemonSetList => app
                ...
                .map(|&index| {
                    if let Some(daemonset) = app.daemonsets.get(index) {
                        ...
                        ListItem::new(format!(
                            "{} - {}/{}",
                            daemonset.name, daemonset.ready, daemonset.desired
                        ))
                        .style(style)
                    } else {
                        ListItem::new("Invalid index").style(Style::default().fg(Color::Red))
            ...
            AppMode::ConfigMapList => app
                ...
                .map(|&index| {
                    if let Some(configmap) = app.configmaps.get(index) {
                        ...
                        ListItem::new(format!("{} - {}", configmap.name, configmap.age))
                            .style(style)
                    } else {
                        ListItem::new("Invalid index").style(Style::default().fg(Color::Red))
                    }
            ...
            _ => {
                vec![
                    ListItem::new("Search not supported for this resource type")
        ...
        if !app.search_results.is_empty() {
            if let Some(pos) = app
                .search_results
                .iter()
                .position(|&idx| match app.previous_mode {
